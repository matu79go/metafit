...
class Dataset(torch.utils.data.Dataset):
    ...
    def _load_person_parts_image(
        self, raw_idx, image, keypoints
    ...
    def __getitem__(self, idx):
        ...
        pose_heatmap, keypoints = self._load_raw_pose_heatmap(
            self._raw_idx[idx]
        )
        head_img, top_img, pant_img, palm_img = self._load_person_parts_image(
            self._raw_idx[idx], person_img, keypoints
        )
...
class ImageFolderDataset(Dataset):
    ...
    def _load_person_parts_image(self, raw_idx, person_img, keypoints):
        ...
        if parsing_path.find("/deepfashion/") > -1:
            parsing_path = parsing_path.replace(
                "/img_320_512_image/", "/img_320_512_parsing/"
            )
        elif parsing_path.find("/mpv/") > -1:
            parsing_path = parsing_path.replace(
                "/MPV_320_512_image/", "/MPV_320_512_parsing/"
            )
        ...
        head_mask = (parsing_label == 2).astype(np.float32) + (
            parsing_label == 13
        ).astype(np.float32)
        top_mask = (
            (parsing_label == 5).astype(np.float32)
            + (parsing_label == 6).astype(np.float32)
            + (parsing_label == 7).astype(np.float32)
            + (parsing_label == 11).astype(np.float32)
        )
        pant_mask = (
            (parsing_label == 8).astype(np.float32)
            + (parsing_label == 9).astype(np.float32)
            + (parsing_label == 12).astype(np.float32)
            + (parsing_label == 18).astype(np.float32)
            + (parsing_label == 19).astype(np.float32)
        )
        ...
        left_hand_up_mask, left_hand_botton_mask = get_hand_mask(
            left_hand_keypoints, height, width
        )
        right_hand_up_mask, right_hand_botton_mask = get_hand_mask(
            right_hand_keypoints, height, width
        )
        # palm mask refined by parsing
        left_hand_mask = parsing_label == 14
        right_hand_mask = parsing_label == 15
        left_palm_mask = get_palm_mask(
            left_hand_mask, left_hand_up_mask, left_hand_botton_mask
        )
        right_palm_mask = get_palm_mask(
            right_hand_mask, right_hand_up_mask, right_hand_botton_mask
        )
    ...
    def _load_raw_pose_heatmap(self, raw_idx):
        ...
        if pose_path.find("/deepfashion/") > -1:
            pose_path = pose_path.replace(
                "/img_320_512_image/", "/img_320_512_keypoints/"
            )
        elif pose_path.find("/mpv/") > -1:
            pose_path = pose_path.replace(
                "/MPV_320_512_image/", "/MPV_320_512_keypoints/"
            )
    ...
    def cords_to_map(self, cords, img_size=(512, 320), sigma=8):
        ...
        for i, point in enumerate(cords):
            ...
            x_matrix, y_matrix = np.meshgrid(
                np.arange(img_size[1]), np.arange(img_size[0])
            )
            ...
            result[..., i] = np.where(
                ((x_matrix - point[0]) ** 2 + (y_matrix - point[1]) ** 2)
                < (sigma**2),
                1,
                0,
            )  # ???�?�?�????��?��??1�?7
    ...
    def get_pose_heatmaps(self, pose_path):
        ...
        with open(pose_path, "r") as f:
            datas = json.load(f)
        keypoints = np.array(datas["people"][0]["pose_keypoints_2d"]).reshape(
            (-1, 3)
        )
...
class UvitonDatasetFull_512(Dataset):
        ...
        **super_kwargs,  # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._path):
            ...
            dataset_list = [
                ...
                "MPV_512_320",
                "ZMO_dresses_512_320",
                "Zalando_512_320_v1_flip",
                "Zalando_512_320_v2_flip",
                "Zalora_512_320_v1_flip",
                "Zalora_512_320_v2_flip",
                "Deepfashion_512_320_flip",
                "MPV_512_320_flip",
                "ZMO_dresses_512_320_flip",
            ]
            ...
            for dataset in dataset_list:
                txt_path = os.path.join(
                    self._path, dataset, "train_pairs_front_list_220508.txt"
                )
                with open(txt_path, "r") as f:
                    for person in f.readlines():
                        person = person.strip().split()[0]
                        self._image_fnames.append(
                            os.path.join(dataset, "image", person)
                        )
                        self._kpt_fnames.append(
                            os.path.join(
                                dataset,
                                "keypoints",
                                person.replace(".jpg", "_keypoints.json"),
                            )
                        )
                        self._garment_parsing_fnames.append(
                            os.path.join(
                                dataset,
                                "garment_parsing",
                                person.replace(".jpg", ".png"),
                            )
                        )
                        if (
                            dataset == "Deepfashion_512_320"
                            or dataset == "MPV_512_320"
                        ):
                            self._parsing_fnames.append(
                                os.path.join(
                                    dataset,
                                    "parsing",
                                    person.replace(".jpg", "_label.png"),
                                )
                            )
                        else:
                            self._parsing_fnames.append(
            ...
            self._image_fnames = [
                self._image_fnames[index] for index in index_list
            ]
            self._kpt_fnames = [
                self._kpt_fnames[index] for index in index_list
            ]
            self._parsing_fnames = [
                self._parsing_fnames[index] for index in index_list
            ]
            self._garment_parsing_fnames = [
                self._garment_parsing_fnames[index] for index in index_list
            ]

            vis_dir = os.path.join(
                self._path, "train_img_front_vis_512_220414"
            )
            image_list = sorted(os.listdir(vis_dir))
            vis_index = []
            for image_name in image_list:
                deepfashion_path = os.path.join(
                    self._path,
                    "Deepfashion_512_320",
                    "image",
                    "train",
                ...
                zalando_path = os.path.join(
                    self._path, "Zalando_512_320_v1", "image", image_name
                )
                zalora_path = os.path.join(
                    self._path, "Zalora_512_320_v2", "image", image_name
                )
                if os.path.exists(zalando_path):
                    vis_index.append(
                        self._image_fnames.index(
                            os.path.join(
                                "Zalando_512_320_v1", "image", image_name
                            )
                        )
                    )
                    ...
                    vis_index.append(
                        self._image_fnames.index(
                            os.path.join(
                                "Zalora_512_320_v2", "image", image_name
                            )
                        )
                    )
            ...
            random_mask_acgpn_dir = os.path.join(
                self._path, "train_random_mask_acgpn"
            )
            self._random_mask_acgpn_fnames = [
                os.path.join(random_mask_acgpn_dir, mask_name)
                for mask_name in os.listdir(random_mask_acgpn_dir)
            ]
    ...
    def _load_raw_image(self, raw_idx):
        ...
        pose, keypoints = self.get_joints(
            kpt
        )  # self.cords_to_map(kpt, im_shape[:2])
        pose = np.pad(
            pose,
            ((0, 0), (left_padding, right_padding), (0, 0)),
            "constant",
        ...
        garment_parsing = np.pad(
            garment_parsing,
            ((0, 0), (left_padding, right_padding), (0, 0)),
            "constant",
            constant_values=(0, 0),
        )
        sleeve_mask = (garment_parsing == 10).astype(np.uint8) + (
            garment_parsing == 11
        ).astype(np.uint8)
        ...
        shoes_mask = (parsing == 18).astype(np.uint8) + (parsing == 19).astype(
            np.uint8
        )
        head_mask = (
            (parsing == 1).astype(np.uint8)
            + (parsing == 2).astype(np.uint8)
            + (parsing == 4).astype(np.uint8)
            + (parsing == 13).astype(np.uint8)
        )
        palm_mask = self.get_palm(keypoints, parsing)
        retain_mask = shoes_mask + palm_mask + head_mask

        hand_leg_mask = (
            (parsing == 14).astype(np.uint8)
        ...
        skin_r_median = np.median(skin_r[skin_r_valid_index]) * np.ones_like(
            image[..., 0:1]
        )
        skin_g_median = np.median(skin_g[skin_g_valid_index]) * np.ones_like(
            image[..., 0:1]
        )
        skin_b_median = np.median(skin_b[skin_b_valid_index]) * np.ones_like(
            image[..., 0:1]
        )
        skin_median = np.concatenate(
            [skin_r_median, skin_g_median, skin_b_median], axis=2
        )

        tops_mask = (parsing == 5).astype(np.uint8) + (parsing == 7).astype(
            np.uint8
        )
        ...
        if np.sum(dresses_mask) > 0:
            if np.sum(lower_pants_mask) > 0:
                ...
                if np.sum(dresses_mask) > (
                    np.sum(tops_mask) + np.sum(lower_skirt_mask)
                ):
        ...
        upper_clothes_mask_rgb = np.concatenate(
            [upper_clothes_mask, upper_clothes_mask, upper_clothes_mask],
            axis=2,
        )
        lower_clothes_mask_rgb = np.concatenate(
            [lower_clothes_mask, lower_clothes_mask, lower_clothes_mask],
            axis=2,
        )
        ...
        lower_clothes_upper_bound_for_train = np.zeros_like(
            lower_clothes_mask[..., 0:1]
        )
        if lower_bbox is not None:
            upper_bound = lower_bbox[1]
            lower_clothes_upper_bound_for_train[upper_bound:, ...] += 255

        lower_clothes_upper_bound_for_test = np.zeros_like(
            lower_clothes_mask[..., 0:1]
        )
        ...
        (
            norm_img,
            norm_img_lower,
            norm_img_lower_for_train,
            denorm_upper_img,
            denorm_lower_img,
            Ms,
            M_invs,
            norm_clothes_masks,
            norm_clothes_masks_lower,
        ) = self.normalize(
            upper_clothes_image,
            lower_clothes_image,
        ...
        return (
            ...
            skin_median,
            lower_label_map,
            lower_clothes_upper_bound_for_train,
            lower_clothes_upper_bound_for_test,
        )
    ...
    def get_palm_mask(self, hand_mask, hand_up_mask, hand_bottom_mask):
        ...
        inter_bottom_mask = ((hand_mask + hand_bottom_mask) == 2).astype(
            np.float32
        )
    ...
    def get_palm(self, keypoints, parsing):
        ...
        left_hand_up_mask, left_hand_botton_mask = self.get_hand_mask(
            left_hand_keypoints
        )
        right_hand_up_mask, right_hand_botton_mask = self.get_hand_mask(
    ...
    def draw_pose_from_cords(
        self, pose_joints, img_size, radius=5, draw_joints=True
    ):
        ...
        if draw_joints:
            for i, p in enumerate(limbseq):
                ...
                fx, fy = int(fx), int(
                    fy
                )  # int(min(fx, 255)), int(min(fy, 191))
                tx, ty = int(tx), int(
                    ty
                )  # int(min(tx, 255)), int(min(ty, 191))
        ...
        for i, joint in enumerate(pose_joints):
            ...
            if i == 9 or i == 10 or i == 12 or i == 13:
                if (
                    (pose_joints[i][0] <= 0)
                    or (pose_joints[i][1] <= 0)
                    or (pose_joints[i][0] >= img_size[1] - 50)
                    or (pose_joints[i][1] >= img_size[0] - 50)
                ):
            ...
            x, y = int(pj[1]), int(
                pj[0]
            )  # int(min(pj[1], 255)), int(min(pj[0], 191))
    ...
    def get_joints(self, keypoints_path):
        ...
        color_joint, keypoints = self.draw_pose_from_cords(
            keypoints, (512, 320)
        )
    ...
    def get_crop(self, keypoints, bpart, order, wh, o_w, o_h, ar=1.0):
        ...
        if part_src.shape[0] == 4:
            ...
            if (
                shoulder_l_new[0] > 0
                and shoulder_l_new[1] > 0
                and shoulder_l_new[0] < o_w
                and shoulder_l_new[1] < o_h
            ):
                part_src[0] = shoulder_l_new
            if (
                shoulder_r_new[0] > 0
                and shoulder_r_new[1] > 0
                and shoulder_r_new[0] < o_w
                and shoulder_r_new[1] < o_h
            ):
    ...
    def normalize(
            ...
            if M is not None:
                if ii == 2 or ii == 3 or ii == 4 or ii == 5:
                    part_img = cv2.warpPerspective(
                        upper_img * sleeve_mask,
                        M,
                        (w, h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )
                    part_clothes_mask = cv2.warpPerspective(
                        upper_clothes_mask * sleeve_mask,
                        M,
                        (w, h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )
                    ...
                    part_img = cv2.warpPerspective(
                        upper_img * (1 - sleeve_mask),
                        M,
                        (w, h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )
                    part_clothes_mask = cv2.warpPerspective(
                        upper_clothes_mask * (1 - sleeve_mask),
                        M,
                        (w, h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )

                denorm_patch = cv2.warpPerspective(
                    part_img, M_inv, (o_w, o_h), borderMode=cv2.BORDER_CONSTANT
                )
                denorm_clothes_mask_patch = cv2.warpPerspective(
                    part_clothes_mask,
                    M_inv,
                    (o_w, o_h),
                    borderMode=cv2.BORDER_CONSTANT,
                )[..., 0:1]
                denorm_clothes_mask_patch = cv2.erode(
                    denorm_clothes_mask_patch, kernel, iterations=1
                )[..., np.newaxis]
                denorm_clothes_mask_patch = (
                    denorm_clothes_mask_patch == 255
                ).astype(np.uint8)

                denorm_upper_img = (
                    denorm_patch * denorm_clothes_mask_patch
                    + denorm_upper_img * (1 - denorm_clothes_mask_patch)
                )

                if ii == 0 or ii >= 6:
                    part_img_lower = cv2.warpPerspective(
                        lower_img, M, (w, h), borderMode=cv2.BORDER_CONSTANT
                    )
                    part_clothes_mask_lower = cv2.warpPerspective(
                        lower_clothes_mask,
                        M,
                        (w, h),
                        borderMode=cv2.BORDER_CONSTANT,
                    ...
                    denorm_patch_lower = cv2.warpPerspective(
                        part_img_lower,
                        M_inv,
                        (o_w, o_h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )
                    denorm_clothes_mask_patch_lower = cv2.warpPerspective(
                        part_clothes_mask_lower,
                        M_inv,
                        (o_w, o_h),
                        borderMode=cv2.BORDER_CONSTANT,
                    )[..., 0:1]
                    denorm_clothes_mask_patch_lower = cv2.erode(
                        denorm_clothes_mask_patch_lower, kernel, iterations=1
                    )[..., np.newaxis]
                    denorm_clothes_mask_patch_lower = (
                        denorm_clothes_mask_patch_lower == 255
                    ).astype(np.uint8)

                    denorm_lower_img = (
                        denorm_patch_lower * denorm_clothes_mask_patch_lower
                        + denorm_lower_img
                        * (1 - denorm_clothes_mask_patch_lower)
                    )
        ...
        if (
            np.sum(left_top_sleeve_mask) == 0
            and np.sum(right_top_sleeve_mask) > 0
        ):
            ...
            np.sum(right_top_sleeve_mask) == 0
            and np.sum(left_top_sleeve_mask) > 0
        ):
        ...
        if (
            np.sum(left_bottom_sleeve_mask) == 0
            and np.sum(right_bottom_sleeve_mask) > 0
        ):
            ...
            np.sum(right_bottom_sleeve_mask) == 0
            and np.sum(left_bottom_sleeve_mask) > 0
        ):
        ...
        if bbox_lower is not None:
            ...
            if random.random() < 0.80:
                if random.random() < 0.6:
                    part_imgs_lower_for_train[0] = np.zeros((h, w, 3)).astype(
                        np.uint8
                    )
    ...
    def __getitem__(self, idx):
        (
            ...
            lower_clothes_upper_bound_for_train,
            lower_clothes_upper_bound_for_test,
        ) = self._load_raw_image(self._raw_idx[idx])
        ...
        lower_clothes_upper_bound_for_train = (
            lower_clothes_upper_bound_for_train.transpose(2, 0, 1)
        )
        lower_clothes_upper_bound_for_test = (
            lower_clothes_upper_bound_for_test.transpose(2, 0, 1)
        )
        ...
        if random.random() < 0.9:
            fname = self._random_mask_acgpn_fnames[
                self._raw_idx[idx] % self._mask_acgpn_numbers
            ]
        ...
        denorm_random_mask_bottom = (denorm_random_mask_bottom > 0).astype(
            np.uint8
        )
        denorm_random_mask_bottom = denorm_random_mask_bottom.transpose(
            2, 0, 1
        )

        denorm_upper_img_erase = denorm_upper_img * (1 - denorm_random_mask)
        denorm_upper_mask = (
            np.sum(denorm_upper_img_erase, axis=0, keepdims=True) > 0
        ).astype(np.uint8)
        denorm_lower_img_erase = denorm_lower_img * (
            1 - denorm_random_mask_bottom
        )
        denorm_lower_mask = (
            np.sum(denorm_lower_img_erase, axis=0, keepdims=True) > 0
        ).astype(np.uint8)
        ...
        return (
            ...
            norm_img.copy(),
            norm_img_lower.copy(),
            norm_img_lower_for_train.copy(),
            denorm_upper_img_erase.copy(),
            denorm_lower_img_erase.copy(),
            Ms.copy(),
            M_invs.copy(),
            gt_parsing.copy(),
            denorm_upper_mask.copy(),
            denorm_lower_mask.copy(),
            norm_clothes_masks.copy(),
            norm_clothes_masks_lower.copy(),
            retain_mask.copy(),
            skin_median.copy(),
            lower_label_map.copy(),
            lower_clothes_upper_bound_for_train.copy(),
...
class UvitonDatasetFull_512_test_full(Dataset):
        ...
        if os.path.isdir(self._path):
            ...
            with open(txt_path, "r") as f:
                for ii, line in enumerate(f.readlines()):
                    ...
                    self._image_fnames.append(
                        os.path.join("image", person_name)
                    )
                    self._kpt_fnames.append(
                        os.path.join(
                            "keypoints", person_name[:-4] + "_keypoints.json"
                        )
                    )
                    self._parsing_fnames.append(
                        os.path.join(
                            "parsing", person_name.replace(".jpg", ".png")
                        )
                    )

                    self._clothes_image_fnames.append(
                        os.path.join("image", clothes_name)
                    )
                    self._clothes_kpt_fnames.append(
                        os.path.join(
                            "keypoints", clothes_name[:-4] + "_keypoints.json"
                        )
                    )
                    self._clothes_garment_parsing_fnames.append(
                        os.path.join(
                            "garment_parsing", clothes_name[:-4] + ".png"
                        )
                    )
                    self._clothes_parsing_fnames.append(
                        os.path.join(
                            "parsing", clothes_name.replace(".jpg", ".png")
                        )
                    )
    ...
    def _load_raw_image(self, raw_idx):
        ...
        kpt = os.path.join(self._path, fname)
        clothes_pose, clothes_keypoints = self.get_joints(
            kpt
        )  # self.cords_to_map(kpt, im_shape[:2])
        clothes_pose = np.pad(
            clothes_pose,
        ...
        tops_mask = (clothes_parsing == 5).astype(np.uint8) + (
            clothes_parsing == 7
        ).astype(np.uint8)
        ...
        denorm_lower_img_mask = (
            np.sum(denorm_lower_img, axis=2, keepdims=True) > 0
        ).astype(np.uint8)
    ...
    def normalize(
        ...
        for ii, bpart in enumerate(bparts):
            ...
            clothes_M, _ = self.get_crop(
                clothes_keypoints, bpart, order, wh, o_w, o_h, ar
            )
            person_M, person_M_inv = self.get_crop(
                person_keypoints, bpart, order, wh, o_w, o_h, ar
            )
    ...
    def __getitem__(self, idx):
        ...
        denorm_lower_img = denorm_lower_img.transpose(2, 0, 1)
        denorm_upper_mask = (
            np.sum(denorm_upper_img, axis=0, keepdims=True) > 0
        ).astype(np.uint8)
        denorm_lower_mask = (
            np.sum(denorm_lower_img, axis=0, keepdims=True) > 0
        ).astype(np.uint8)

        skin_average = skin_average.transpose(2, 0, 1)
        retain_mask = retain_mask.transpose(2, 0, 1)
        lower_label_map = lower_label_map.transpose(2, 0, 1)
        lower_clothes_upper_bound = lower_clothes_upper_bound.transpose(
            2, 0, 1
        )
        ...
        return (
            ...
            norm_img.copy(),
            norm_img_lower.copy(),
            denorm_upper_img.copy(),
            denorm_lower_img.copy(),
            denorm_upper_mask.copy(),
            denorm_lower_mask.copy(),
            retain_mask.copy(),
            skin_average.copy(),
            lower_label_map.copy(),
            lower_clothes_upper_bound.copy(),
            person_name,
            clothes_name,
        )
...
class UvitonDatasetFull_512_test_upper(Dataset):
    ...
    def _load_raw_image(self, raw_idx):
        ...
        clothes_tops_mask = (clothes_parsing == 5).astype(np.uint8) + (
            clothes_parsing == 7
        ).astype(np.uint8)
        ...
        if np.sum(clothes_dresses_mask) > 0:
            if np.sum(clothes_lower_pants_mask) > 0:
                ...
                if np.sum(clothes_dresses_mask) > (
                    np.sum(clothes_tops_mask)
                    + np.sum(clothes_lower_skirt_mask)
                ):
                    clothes_dresses_mask += (
                        clothes_tops_mask + clothes_lower_skirt_mask
                    )
                    clothes_tops_mask *= 0
                    clothes_lower_skirt_mask *= 0
                else:
                    if np.sum(clothes_tops_mask) > np.sum(
                        clothes_lower_skirt_mask
                    ):
        ...
        denorm_lower_img_mask = cv2.erode(
            lower_clothes_mask_rgb, kernel, iterations=1
        )[..., 0:1]
        ...
        denorm_upper_img_mask = (
            np.sum(denorm_upper_img_wo_sleeve, axis=2, keepdims=True) > 0
        ).astype(np.uint8)
    ...
    def normalize(
        ...
        for ii, bpart in enumerate(bparts):
            ...
            clothes_M, clothes_M_inv = self.get_crop(
                clothes_keypoints, bpart, order, wh, o_w, o_h, ar
            )
            person_M, person_M_inv = self.get_crop(
                person_keypoints, bpart, order, wh, o_w, o_h, ar
        ...
        upper_torso_mask = (
            np.sum(part_clothes_masks[0], axis=2, keepdims=True) > 0
        ).astype(np.uint8)
        upper_left_hip_mask = (
            np.sum(part_clothes_masks[6], axis=2, keepdims=True) > 0
        ).astype(np.uint8)
        upper_right_hip_mask = (
            np.sum(part_clothes_masks[8], axis=2, keepdims=True) > 0
        ).astype(np.uint8)
        ...
        part_clothes_masks_lower[0] = part_clothes_masks_lower[0] * (
            1 - upper_torso_mask
        )
        part_clothes_masks_lower[1] = part_clothes_masks_lower[1] * (
            1 - upper_left_hip_mask
        )
        part_clothes_masks_lower[3] = part_clothes_masks_lower[3] * (
            1 - upper_right_hip_mask
        )
...
class UvitonDatasetFull_512_test_lower(Dataset):
        ...
        if os.path.isdir(self._path):
            ...
            with open(txt_path, "r") as f:
                for ii, line in enumerate(f.readlines()):
                    ...
                    self._garment_parsing_fnames.append(
                        os.path.join(
                            "garment_parsing", person_name[:-4] + ".png"
                        )
                    )
                    self._parsing_fnames.append(
                        os.path.join(
    ...
    def _load_raw_image(self, raw_idx):
        ...
        kernel = np.ones((8, 8), dtype=np.uint8)
        denorm_upper_img_mask = cv2.erode(
            upper_clothes_mask_rgb, kernel, iterations=1
        )[..., 0:1]
