...
from util_functions import feature_normalize
...
class Loss:
    def accumulate_gradients(
        self, phase, real_img, gen_z, real_c, retain, sync, gain
...
class StyleGAN2Loss(Loss):
    def __init__(
        ...
        class_weight = torch.FloatTensor([1, 3, 4, 4, 4, 4, 4]).to(device)
        self.ce_parsing = torch.nn.CrossEntropyLoss(
            ignore_index=255, weight=class_weight
        )
        ...
        if self.contextual_weight > 0:
            ...
            self.contextual_vgg.load_state_dict(
                torch.load(contextual_vgg_path)
            )
    ...
    def run_G(
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img, finetune_img, pred_parsing = self.G_synthesis(
                ws,
                pose_feat,
                cat_feats,
                denorm_upper_input,
    ...
    def run_D(self, img, pose, c, sync):
        ...
        with misc.ddp_sync(self.D, sync):
            logits = self.D(torch.cat([img, pose[:, 0:3, ...]], dim=1), c)
        return logits

    def run_D_parsing(self, parsing, pose, c, sync):
        # if self.augment_pipe is not None:
        #     parsing = self.augment_pipe(parsing)

        with misc.ddp_sync(self.D_parsing, sync):
            logits = self.D_parsing(
                torch.cat([parsing, pose[:, 0:3, ...]], dim=1), c
            )
        return logits
    ...
    def accumulate_gradients(
        ...
        do_DPmain = phase in ["D_parsingmain", "D_parsingboth"]
        do_DPr1 = (phase in ["D_parsingreg", "D_parsingboth"]) and (
            self.r1_gamma != 0
        )
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function("Gmain_forward"):
                gen_img, gen_finetune_img, pred_parsing, _ = self.run_G(
                    gen_z,
                    gen_c,
                    pose,
                    cat_feats,
                ...
                pred_parsing_onehot = self.softmax(pred_parsing)
                ...
                gen_logits = self.run_D(gen_img, pose, gen_c, sync=False)
                gen_finetune_logits = self.run_D(
                    gen_finetune_img, pose, gen_c, sync=False
                )
                parsing_logits = self.run_D_parsing(
                    pred_parsing_onehot, pose, gen_c, sync=False
                )
                ...
                training_stats.report(
                    "Loss/scores/fake_finetune", gen_finetune_logits
                )
                training_stats.report(
                    "Loss/signs/fake_finetune", gen_finetune_logits.sign()
                )
                loss_Gmain_finetune = torch.nn.functional.softplus(
                    -gen_finetune_logits
                )  # -log(sigmoid(gen_logits))
                loss_Gmain_finetune = loss_Gmain_finetune.mean()
                training_stats.report(
                    "Loss/G/loss_finetune", loss_Gmain_finetune
                )

                training_stats.report(
                    "Loss/scores/fake_parsing", parsing_logits
                )
                training_stats.report(
                    "Loss/signs/fake_parsing", parsing_logits.sign()
                )
                loss_Gmain_parsing = torch.nn.functional.softplus(
                    -parsing_logits
                )  # -log(sigmoid(gen_logits))
                loss_Gmain_parsing = loss_Gmain_parsing.mean()
                training_stats.report(
                    "Loss/G/loss_parsing", loss_Gmain_parsing
                )
                ...
                if self.l1_weight > 0:
                    loss_G_L1 = (
                        torch.nn.L1Loss()(gen_img, real_img) * self.l1_weight
                    )
                    loss_G_finetune_L1 = (
                        torch.nn.L1Loss()(gen_finetune_img, real_img)
                        * self.l1_weight
                    )
                ...
                if self.mask_weight > 0:
                    loss_mask = (
                        torch.mean(
                            self.ce_parsing(
                                pred_parsing, gt_parsing.long()[:, 0, ...]
                            )
                        )
                        * self.mask_weight
                    )
                ...
                if self.vgg_weight > 0:
                    loss_G_VGG = (
                        self.criterionVGG(gen_img, real_img) * self.vgg_weight
                    )
                    loss_G_VGG = loss_G_VGG.mean()

                    loss_G_finetune_VGG = (
                        self.criterionVGG(gen_finetune_img, real_img)
                        * self.vgg_weight
                    )
                    loss_G_finetune_VGG = loss_G_finetune_VGG.mean()
                training_stats.report("Loss/G/vgg", loss_G_VGG)
                training_stats.report(
                    "Loss/G/vgg_finetune", loss_G_finetune_VGG
                )

                loss_G = (
                    (loss_Gmain + loss_Gmain_finetune) / 2
                    + (loss_G_L1 + loss_G_finetune_L1) / 2
                    + (loss_G_VGG + loss_G_finetune_VGG) / 2
                    + loss_mask
                    + loss_Gmain_parsing
                )
                # loss_G = loss_Gmain_finetune + loss_G_finetune_L1 + \
                #  loss_G_finetune_VGG + loss_mask + loss_Gmain_parsing
        ...
        # loss_Dgen = 0
        loss_Dgen_finetune = 0
        if do_Dmain:
            with torch.autograd.profiler.record_function("Dgen_forward"):
                gen_img, gen_finetune_img, _, _gen_ws = self.run_G(
                    gen_z,
                    gen_c,
                    pose,
                    cat_feats,
                ...
                loss_Dgen_finetune = torch.nn.functional.softplus(
                    gen_finetune_logits
                )  # -log(1 - sigmoid(gen_logits))

            with torch.autograd.profiler.record_function("Dgen_backward"):
                ((loss_Dgen.mean() + loss_Dgen_finetune.mean()) / 2).mul(
                    gain
                ).backward()
            # with torch.autograd.profiler.record_function('Dgen_backward'):
            # (loss_Dgen_finetune.mean()).mul(gain).backward()
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + "_forward"):
                ...
                real_logits = self.run_D(real_img_tmp, pose, real_c, sync=sync)
                ...
                if do_Dmain:
                    ...
                    training_stats.report(
                        "Loss/D/loss", loss_Dgen_finetune + loss_Dreal
                    )
        ...
        # Dparsingmain: Minimize logits for generated images.
        loss_Dparsing = 0
        if do_DPmain:
            with torch.autograd.profiler.record_function("Dparsing_forward"):
                _, _, pred_parsing, _ = self.run_G(
                    gen_z,
                    gen_c,
                    pose,
                    cat_feats,
                ...
                loss_Dparsing = torch.nn.functional.softplus(
                    parsing_logits
                )  # -log(1 - sigmoid(gen_logits))
                # loss_Dparsing = loss_Dparsing * 0.5

            with torch.autograd.profiler.record_function("Dparsing_backward"):
                loss_Dparsing.mean().mul(gain).backward()

        # DParsingmain: Maximize logits for real images.
        # DParsingr1: Apply R1 regularization.
        if do_DPmain or do_DPr1:
            name = (
                "DPreal_DPr1"
                if do_DPmain and do_DPr1
                else "DPreal"
                if do_DPmain
                else "DPr1"
            )
            with torch.autograd.profiler.record_function(name + "_forward"):
                ...
                gt_parsing_tmp = (
                    ...
                    .detach()
                    .requires_grad_(do_DPr1)
                )
                real_parsing_logit = self.run_D_parsing(
                    gt_parsing_tmp, pose, real_c, sync=sync
                )
                training_stats.report(
                    "Loss/scores/real_parsing", real_parsing_logit
                )
                training_stats.report(
                    "Loss/signs/real_parsing", real_parsing_logit.sign()
                )

                loss_DPreal = 0
                if do_DPmain:
                    loss_DPreal = torch.nn.functional.softplus(
                        -real_parsing_logit
                    )  # -log(sigmoid(real_logits))
                    # loss_DPreal = loss_DPreal * 0.5
                    training_stats.report(
                        "Loss/D_parsing/loss", loss_Dparsing + loss_DPreal
                    )
                ...
                if do_DPr1:
                    with torch.autograd.profiler.record_function(
                        ...
                        DP_r1_grads = torch.autograd.grad(
                            outputs=[real_parsing_logit.sum()],
                            inputs=[gt_parsing_tmp],
                            create_graph=True,
                            only_inputs=True,
                        )[0]
                    DP_r1_penalty = DP_r1_grads.square().sum([1, 2, 3])
                    loss_DPr1 = DP_r1_penalty * (self.r1_gamma / 2)
                    # loss_DPr1 = loss_DPr1 * 0.5
                    training_stats.report("Loss/DP_r1_penalty", DP_r1_penalty)
                    training_stats.report("Loss/D/reg", loss_DPr1)

            with torch.autograd.profiler.record_function(name + "_backward"):
                (real_parsing_logit * 0 + loss_DPreal + loss_DPr1).mean().mul(
                    gain
                ).backward()
...
class VGG19_Feature(torch.nn.Module):
    def __init__(
        self,
        device,
        ckpt_path="./checkpoints/vgg19-dcbb9e9d.pth",
        requires_grad=False,
    ):
        ...
        vgg_pretrained_features = vgg19(
            pretrained=True, progress=False, ckpt_path=ckpt_path
        ).features  # load from local file
...
def vgg19(
    ...
    progress: bool = True,
    ckpt_path: str = "./checkpoints/vgg19-dcbb9e9d.pth",
    **kwargs: Any
) -> VGG:
...
class ContextualLoss_forward(nn.Module):
    ...
    def forward(self, X_features, Y_features, h=0.1, feature_centering=True):
        ...
        if feature_centering:
            if self.PONO:
                X_features = X_features - Y_features.mean(dim=1).unsqueeze(
                    dim=1
                )
                Y_features = Y_features - Y_features.mean(dim=1).unsqueeze(
                    dim=1
                )
            else:
        ...
        # d_norm = d
        d_norm = d / (
            torch.min(d, dim=-1, keepdim=True)[0] + 1e-3
        )  # batch_size * feature_size^2 * feature_size^2
