...
def denorm_clothes(
    ...
    norm_patches_lower,
    norm_clothes_mask,
    norm_clothes_mask_lower,
    gt_parsings,
    lower_label_maps,
    lower_clothes_upper_bounds_for_test,
    Ms,
    M_invs,
    col,
    row,
    ...
    for ii in range(M_invs.shape[1]):
        ...
        if row < gap:
            norm_patch = norm_patches[
                row, ii * 3 : (ii + 1) * 3, ...
            ].transpose(1, 2, 0)
            norm_clothes_mask_patch = norm_clothes_mask[
                row, ii * 3 : (ii + 1) * 3, ...
            ].transpose(1, 2, 0)
        ...
        if ii == 0:
            ...
            if row < 2 * gap:
                norm_patch_lower = norm_patches_lower[
                    col, ii * 3 : (ii + 1) * 3, ...
                ].transpose(1, 2, 0)
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[
                    col, ii * 3 : (ii + 1) * 3, ...
                ].transpose(1, 2, 0)
        ...
        denorm_patch = cv2.warpPerspective(
            norm_patch, M_inv, (512, 512), borderMode=cv2.BORDER_CONSTANT
        )
        denorm_clothes_mask_patch = cv2.warpPerspective(
            norm_clothes_mask_patch,
            M_inv,
            (512, 512),
        ...
        denorm_clothes_mask_patch = (denorm_clothes_mask_patch == 255).astype(
            np.uint8
        )
        denorm_upper_img = (
        ...
        if ii == 0 or ii >= 6:
            ...
            denorm_lower_img = (
                ...
                + denorm_lower_img * (1 - denorm_clothes_mask_patch_lower)
            )

        upper_norm_patch_list.append(norm_patch)
        if ii == 0 or ii >= 6:
            norm_clothes_mask_patch_tmp = norm_clothes_mask_patch[..., 0:1]
            norm_clothes_mask_patch_tmp = (
                norm_clothes_mask_patch_tmp > 0
            ).astype(np.uint8)
            norm_patch_lower_tmp = norm_patch_lower * (
                1 - norm_clothes_mask_patch_tmp
            )
            denorm_patch_tmp = cv2.warpPerspective(
                norm_patch_lower_tmp,
                M_inv,
                (512, 512),
                borderMode=cv2.BORDER_CONSTANT,
            )
            norm_patch_lower_tmp = cv2.warpPerspective(
                denorm_patch_tmp, M, (128, 128), borderMode=cv2.BORDER_CONSTANT
            )
    ...
    denorm_upper_clothes_mask = (
        np.sum(denorm_upper_img, axis=1, keepdims=True) > 0
    ).astype(np.uint8)
    denorm_lower_clothes_mask = (
        np.sum(denorm_lower_img, axis=1, keepdims=True) > 0
    ).astype(np.uint8)
    ...
    upper_lower_norm_patches = np.concatenate(
        [upper_norm_patches, lower_norm_patches], axis=2
    )
    upper_lower_norm_patches = upper_lower_norm_patches.transpose(2, 0, 1)[
        np.newaxis, ...
    ]

    if row < gap:
        gt_parsing = gt_parsings[row].transpose(1, 2, 0)
        lower_mask = (gt_parsing == 2).astype(np.uint8) + (
            gt_parsing == 3
        ).astype(np.uint8)
        ...
        if np.sum(M_inv_0) != 0:
            denorm_patch_lower_0 = cv2.warpPerspective(
                norm_patch_lower_0,
                M_inv_0,
                (512, 512),
                borderMode=cv2.BORDER_CONSTANT,
        ...
        if np.sum(M_inv_6) != 0:
            denorm_patch_lower_6 = cv2.warpPerspective(
                norm_patch_lower_6,
                M_inv_6,
                (512, 512),
                borderMode=cv2.BORDER_CONSTANT,
        ...
        if np.sum(M_inv_8) != 0:
            denorm_patch_lower_8 = cv2.warpPerspective(
                norm_patch_lower_8,
                M_inv_8,
                (512, 512),
                borderMode=cv2.BORDER_CONSTANT,
        ...
        denorm_lower_mask_tmp = (
            np.sum(denorm_lower_img_tmp, axis=2, keepdims=True) > 0
        ).astype(np.uint8)
        ...
        lower_clothes_upper_bound = lower_clothes_upper_bounds_for_test[
            row
        ].transpose(1, 2, 0)
        norm_patch_torso = upper_norm_patch_list[0]
        M_inv = M_invs[row, 0]
        denorm_patch_torso = cv2.warpPerspective(
            norm_patch_torso, M_inv, (512, 512), borderMode=cv2.BORDER_CONSTANT
        )
        denorm_patch_torso_mask = (
            np.sum(denorm_patch_torso, axis=2, keepdims=True) > 0
        ).astype(np.uint8)
    ...
    lower_clothes_conditions = np.concatenate(
        [lower_label_map, lower_clothes_upper_bound], axis=2
    )
    lower_clothes_conditions = lower_clothes_conditions.transpose(2, 0, 1)[
        np.newaxis, ...
    ]
    ...
    return (
        ...
        denorm_upper_clothes_mask,
        denorm_lower_clothes_mask,
        upper_lower_norm_patches,
        lower_clothes_conditions,
    )
...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    (
        ...
        _,
        lower_clothes_upper_bounds_for_test,
    ) = zip(*[training_set[i] for i in grid_indices])
    ...
    lower_clothes_upper_bounds_for_test = np.array(
        lower_clothes_upper_bounds_for_test
    )
    ...
    (
        grid_denorm_upper_ims,
        grid_denorm_lower_ims,
        grid_denorm_upper_masks,
        grid_denorm_lower_masks,
        grid_norm_parts,
        grid_lower_clothes_conditions,
    ) = zip(
        *[
            denorm_clothes(
                norm_patches,
    ...
    grid_denorm_upper_ims = (
        torch.from_numpy(grid_denorm_upper_ims).to(device).to(torch.float32)
        / 127.5
        - 1
    )
    ...
    grid_denorm_lower_ims = (
        torch.from_numpy(grid_denorm_lower_ims).to(device).to(torch.float32)
        / 127.5
        - 1
    )
    ...
    grid_denorm_upper_masks = (
        torch.from_numpy(grid_denorm_upper_masks).to(device).to(torch.float32)
    )

    grid_denorm_lower_masks = np.concatenate(grid_denorm_lower_masks, axis=0)
    grid_denorm_lower_masks = (
        torch.from_numpy(grid_denorm_lower_masks).to(device).to(torch.float32)
    )

    grid_norm_parts = np.concatenate(grid_norm_parts, axis=0)
    grid_norm_parts = (
        torch.from_numpy(grid_norm_parts).to(device).to(torch.float32) / 127.5
        - 1
    )

    grid_lower_clothes_conditions = np.concatenate(
        grid_lower_clothes_conditions, axis=0
    )
    grid_lower_clothes_conditions = (
        torch.from_numpy(grid_lower_clothes_conditions)
        .to(device)
        .to(torch.float32)
        / 127.5
        - 1
...
def training_loop(
    ...
    common_kwargs_D = dict(
        c_dim=512,
        img_resolution=training_set.resolution,
        img_channels=(training_set.num_channels + 3),
    )
    common_kwargs_D_parsing = dict(
        c_dim=512, img_resolution=training_set.resolution, img_channels=(7 + 3)
    )
    G = (
    ...
    D = (
        dnnlib.util.construct_class_by_name(**D_kwargs, **common_kwargs_D)
        .train()
        .requires_grad_(False)
        .to(device)
    )  # subclass of torch.nn.Module
    D_parsing = (
        dnnlib.util.construct_class_by_name(
            **D_kwargs, **common_kwargs_D_parsing
        )
        .train()
        .requires_grad_(False)
        .to(device)
    ...
    if rank == 0:
        ...
        const_feature = torch.empty(
            [batch_gpu, 3 + 3, 512, 512], device=device
        )
        ...
        misc.print_module_summary(
            D, [torch.cat([img[0], pose[:, 0:3, ...]], dim=1), c_feature]
        )
        misc.print_module_summary(
            D_parsing,
            [torch.cat([img[2], pose[:, 0:3, ...]], dim=1), c_feature],
        )
    ...
    for name, module in [
        ...
        ("G_const_encoding", G.const_encoding),
        ("G_style_encoding", G.style_encoding),
        ("D", D),
        ("D_parsing", D_parsing),
        (None, G_ema),
    ...
    for name, module, opt_kwargs, reg_interval in [
        ...
        if reg_interval is None:
            ...
            phases += [
                dnnlib.EasyDict(
                    name=name + "both", module=module, opt=opt, interval=1
                )
            ]
            ...
            phases += [
                dnnlib.EasyDict(
                    name=name + "main", module=module, opt=opt, interval=1
                )
            ]
            phases += [
                dnnlib.EasyDict(
    ...
    if rank == 0:
        ...
        (
            ...
            grid_part,
            grid_retain,
            grid_denorm_upper_im,
            grid_denorm_lower_im,
            grid_denorm_upper_mask,
            grid_denorm_lower_mask,
        ...
        image_side = (
            source_im.unsqueeze(1)
            .numpy()
            .transpose(0, 3, 1, 4, 2)
            .reshape(gnum * H, 1 * W, C)
        )
        image_top = torch.cat(
            (torch.zeros(grid_im[0].shape).unsqueeze(0), source_im), dim=0
        )
        image_top = (
            image_top.unsqueeze(0)
            .numpy()
            .transpose(0, 3, 1, 4, 2)
            .reshape(1 * H, (gnum + 1) * W, C)
        )
        ...
        grid_z = torch.randn(
            [grid_retain.shape[0], G.z_dim], device=device
        ).split(batch_gpu)
        ...
        save_image_grid(
            ...
            image_top,
            upper_images,
            os.path.join(run_dir, "init_denorm_upper.png"),
            drange=[-1, 1],
            grid_size=(gnum, gnum),
        )
        save_image_grid(
            ...
            image_top,
            lower_images,
            os.path.join(run_dir, "init_denorm_lower.png"),
            drange=[-1, 1],
            grid_size=(gnum, gnum),
        )
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function("data_fetch"):
            ...
            (
                ...
                phase_norm_img,
                _,
                phase_norm_img_lower,
                phase_denorm_upper_img,
                phase_denorm_lower_img,
                _,
                _,
                phase_gt_parsing,
                phase_denorm_upper_mask,
                phase_denorm_lower_mask,
                _,
                _,
                phase_retain_mask,
                phase_skin,
                phase_lower_label_map,
                phase_lower_clothes_upper_bound,
                _,
            ) = next(training_set_iterator)

            phase_real_tensor = (
                phase_real.to(device).to(torch.float32) / 127.5 - 1
            )

            phase_lower_label_map_tensor = (
                phase_lower_label_map.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_lower_clothes_upper_bound_tensor = (
                phase_lower_clothes_upper_bound.to(device).to(torch.float32)
                / 127.5
                - 1
            )
            phase_skin_tensor = (
                phase_skin.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_retain_mask = phase_retain_mask.to(device)
            phase_parts_tensor = (
                phase_norm_img.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_parts_lower_tensor = (
                phase_norm_img_lower.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_parts_tensor = torch.cat(
                [phase_parts_tensor, phase_parts_lower_tensor], dim=1
            )

            phase_denorm_upper_img_tensor = (
                phase_denorm_upper_img.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_denorm_lower_img_tensor = (
                phase_denorm_lower_img.to(device).to(torch.float32) / 127.5 - 1
            )

            phase_denorm_upper_mask_tensor = phase_denorm_upper_mask.to(
                device
            ).to(torch.float32)
            phase_denorm_lower_mask_tensor = phase_denorm_lower_mask.to(
                device
            ).to(torch.float32)

            phase_pose_tensor = (
                phase_pose.to(device).to(torch.float32) / 127.5 - 1
            )
            phase_pose_tensor = torch.cat(
                (
                    phase_pose_tensor,
                    phase_lower_label_map_tensor,
                    phase_lower_clothes_upper_bound_tensor,
            ...
            phase_gt_parsing_tensor = phase_gt_parsing.to(device).to(
                torch.float32
            )

            # process head
            phase_head_mask = phase_retain_mask
            phase_head_tensor = phase_head_mask * phase_real_tensor - (
                1 - phase_head_mask
            )
            phase_retain_tensor = torch.cat(
                [phase_head_tensor, phase_skin_tensor], dim=1
            )
            ...
            phase_denorm_upper_img_tensor = (
                phase_denorm_upper_img_tensor.split(batch_gpu)
            )
            phase_denorm_lower_img_tensor = (
                phase_denorm_lower_img_tensor.split(batch_gpu)
            )

            phase_gt_parsing_tensor = phase_gt_parsing_tensor.split(batch_gpu)

            phase_denorm_upper_mask_tensor = (
                phase_denorm_upper_mask_tensor.split(batch_gpu)
            )
            phase_denorm_lower_mask_tensor = (
                phase_denorm_lower_mask_tensor.split(batch_gpu)
            )
        ...
        for phase, phase_gen_z in zip(phases, all_gen_z):
            ...
            for round_idx, (
                ...
                loss.accumulate_gradients(
                    ...
                    retain=retain,
                    pose=pose,
                    denorm_upper_input=denorm_upper_input,
                    denorm_lower_input=denorm_lower_input,
                    denorm_upper_mask=denorm_upper_mask,
                    denorm_lower_mask=denorm_lower_mask,
                    gt_parsing=gt_parsing,
                    sync=sync,
                    gain=gain,
                )
        ...
        if (
            ...
            for (
                ...
                _, finetune_fake_image, pred_parsing = G_ema(
                    z=z,
                    c=c,
                    retain=retain,
                    pose=pose,
                    denorm_upper_input=denorm_upper_im,
                    denorm_lower_input=denorm_lower_im,
                    denorm_upper_mask=denorm_upper_ma,
                    denorm_lower_mask=denorm_lower_ma,
                    noise_mode="const",
                )
                finetune_fake_images.append(finetune_fake_image.cpu().numpy())

                softmax = torch.nn.Softmax(dim=1)
                parsing_index = torch.argmax(softmax(pred_parsing), dim=1)[
                    :, None, ...
                ].float()
                parsing_index = parsing_index.cpu().numpy()
                parsing_index = np.concatenate(
                    [parsing_index, parsing_index, parsing_index], axis=1
                )
            ...
            save_image_grid(
                ...
                finetune_fake_images,
                os.path.join(
                    run_dir, f"fakes{cur_nimg//1000:06d}_finetune.png"
                ),
                drange=[-1, 1],
                grid_size=(gnum, gnum),
            )
            save_image_grid(
                ...
                pred_parsings,
                os.path.join(
                    run_dir, f"fakes{cur_nimg//1000:06d}_parsing.png"
                ),
                drange=[-1, 1],
                grid_size=(gnum, gnum),
            )
