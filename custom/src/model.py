...
def modulated_conv2d(
    ...
    flip_weight=True,
    fused_modconv=True,  # Perform modulation, convolution, and
    # demodulation as a single fused operation?
):
...
class Conv2dLayer(nn.Module):
    def __init__(
        ...
        channels_last=False,  # Expect the input to have
...
class MappingNetwork(nn.Module):
    def __init__(
        ...
        embed_features=None,  # Label embedding dimensionality,
        # None = same as w_dim.
...
class FeatureEncoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=7):
        ...
        for i in range(n_downsampling):
            ...
            encoder += [
                Conv2dLayer(
                    ngf * mult_in, ngf * mult_out, kernel_size=3, down=2
                )
            ]
...
# @persistence.persistent_class
# class ResBlock_partialconv(nn.Module):
#     def __init__(
#         self,
#         in_channels,  # Number of input channels.
#         out_channels,  # Number of output channels.
...
#         self.down = down
#
#         self.conv0 = Conv2dLayer_partialconv(
#             in_channels,
#             out_channels,
#             kernel_size=3,
#             activation=activation,
...
#             channels_last=channels_last,
#         )
#         self.conv1 = Conv2dLayer_partialconv(
#             out_channels,
#             out_channels,
#             kernel_size=3,
#             activation=activation,
...
class SpaceToDepth(nn.Module):
    ...
    def forward(self, x):
        ...
        unfolded_x = torch.nn.functional.unfold(
            x, self.block_size, stride=self.block_size
        )
...
class DiscriminatorEpilogue(nn.Module):
    def __init__(
        ...
        img_channels,  # Number of input color channels.
        architecture="resnet",  # Architecture: 'orig', 'skip', 'resnet'.
        # Group size for the minibatch standard deviation layer,
...
class RandomSpatialTransformer:
    ...
    def resample_transformation(
        ...
        if rotation is None:
            ...
            rot = (
                torch.rand((bs), device=dev) * (2 * max_rotation)
                - max_rotation
            )
            # else:
            #    rot = 0.0
        ...
        if scale is None:
            ...
            max_scale = 1.0
            sx = (
                torch.rand((bs), device=dev) * (max_scale - min_scale)
                + min_scale
            )
            sy = (
                torch.rand((bs), device=dev) * (max_scale - min_scale)
...
class EqualConv2d(nn.Module):
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}({self.weight.shape[1]}, {self.weight.shape[0]},"  # noqa
            f" {self.weight.shape[2]}, stride={self.stride}, padding={self.padding})"  # noqa
        )
...
class ResBlock_PD(nn.Module):
    def __init__(
        ...
        self.conv1 = ConvLayer(
            in_channel, in_channel, 3, reflection_pad=reflection_pad, pad=pad
        )
...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            if input.dim() > 2:
                out = F.conv2d(
                    input, self.weight[:, :, None, None] * self.scale
                )
            else:
            ...
            if input.dim() > 2:
                out = F.conv2d(
                    input,
                    self.weight[:, :, None, None] * self.scale,
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.weight.shape[1]}, {self.weight.shape[0]})"  # noqa
    ...
    def fix_and_gather_noise_parameters(self):
        ...
        for m in self.modules():
            if type(m).__name__ == "NoiseInjection":
                assert (
                    m.image_size is not None
                ), "One forward call should be made to determine size of noise parameters"  # noqa
                m.fixed_noise = torch.nn.Parameter(
                    torch.randn(
                        m.image_size[0],
                        1,
                        m.image_size[2],
...
class BasePatchDiscriminator(BaseNetwork):
    ...
    def apply_random_transformation(self, patches):
        ...
        patches = transformer.forward_transform(
            patches, (self.opt.patch_size, self.opt.patch_size)
        )
    ...
    def sample_patches(self, img, indices):
        ...
        if indices is None:
            indices = torch.randperm(ntiles, device=img.device)[
                : self.opt.max_num_tiles
            ]
    ...
    def forward(self, real, fake, fake_only=False):
        ...
        if fake is None or not fake_only:
            pred_real = self.discriminate_features(
                real_feat, torch.roll(real_feat, 1, 1)
            )
        ...
        if fake is not None:
            ...
            pred_fake = self.discriminate_features(
                real_feat, torch.roll(fake_feat, 1, 1)
            )
...
class StyleGAN2PatchDiscriminator(BasePatchDiscriminator):
    ...
    def __init__(self, **kwargs):
        ...
        for i in range(log_size, 2, -1):
            ...
            convs.append(
                (layer_name, ResBlock_PD(in_channel, out_channel, blur_kernel))
            )
        ...
        convs.append(
            (
                "5",
                ResBlock_PD(
                    in_channel, self.opt.netPatchD_max_nc * 2, downsample=False
                ),
            )
        )
        convs.append(
            (
                "6",
                ConvLayer(
                    self.opt.netPatchD_max_nc * 2,
                    self.opt.netPatchD_max_nc,
                    3,
                    pad=0,
                ),
            )
        ...
        pairlinear1 = EqualLinear(
            channels[4] * 2 * 2 * 2, 2048, activation="fused_lrelu"
        )
        ...
        self.pairlinear = nn.Sequential(
            pairlinear1, pairlinear2, pairlinear3, pairlinear4
        )
    ...
    def extract_features(self, patches, aggregate=False):
        ...
        features = features.view(
            B, T, features.size(1), features.size(2), features.size(3)
        )
        if aggregate:
...
# only user patch as the inputs of the discriminator,
# do not use the average real feature @@@@
@persistence.persistent_class
class StyleGAN2PatchDiscriminator_V2(BasePatchDiscriminator):
    def __init__(self, **kwargs):
        ...
        pairlinear1 = EqualLinear(
            channels[4] * 2 * 2, 2048, activation="fused_lrelu"
        )
        pairlinear2 = EqualLinear(2048, 2048, activation="fused_lrelu")
        pairlinear3 = EqualLinear(2048, 1024, activation="fused_lrelu")
        pairlinear4 = EqualLinear(1024, out_dim)
...
def spade_modulated_conv2d(
    ...
    if spade_styles is not None:
        misc.assert_shape(
            spade_styles, [batch_size, in_channels, x.shape[2], x.shape[3]]
        )
    ...
    if spade_styles is not None:
        x = (
            x
            * (
                spade_styles.to(x.dtype)
                + styles.to(x.dtype).reshape(batch_size, -1, 1, 1)
            )
            / 2
        )
    else:
...
@persistence.persistent_class
class Spade_Conv2dLayer_partialconv(nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels.
        out_channels,  # Number of output channels.
        ...
        mask_weight = torch.ones([1, 1, kernel_size, kernel_size]).to(
    ...
    def forward(self, x, mask, gain=1, no_act=False):
        ...
        mask_inverse = mask == 0
        x_mask = conv2d_resample.conv2d_resample(
            x=mask,
            w=mask_w.to(x.dtype),
            f=self.resample_filter,
            up=self.up,
...
class Spade_Norm_Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv_mlp = Spade_Conv2dLayer(
            in_channels, norm_channels, kernel_size=3, bias=False
        )
        self.conv_mlp_act = nn.ReLU()
        self.conv_gamma = Spade_Conv2dLayer(
            norm_channels, norm_channels, kernel_size=3, bias=False
        )
        self.conv_beta = Spade_Conv2dLayer(
            norm_channels, norm_channels, kernel_size=3, bias=False
        )
...
class ToRGBLayerV18(nn.Module):
    def __init__(
        ...
        if self.is_last:
            self.m_weight1 = torch.nn.Parameter(
                torch.randn([1, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
            self.m_bias1 = torch.nn.Parameter(torch.zeros([1]))

            self.m_weight2 = torch.nn.Parameter(
                torch.randn([1, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
    ...
    def forward(self, x, w, fused_modconv=True):
        ...
        lower_mask = None
        if self.is_last:
            upper_mask = modulated_conv2d(
                x=x,
                weight=self.m_weight1,
                styles=styles,
                demodulate=False,
                fused_modconv=fused_modconv,
            )
            upper_mask = bias_act.bias_act(
                upper_mask,
                self.m_bias1.to(x.dtype),
                clamp=self.conv_clamp,
                act="sigmoid",
            )

            lower_mask = modulated_conv2d(
                x=x,
                weight=self.m_weight2,
                styles=styles,
                demodulate=False,
                fused_modconv=fused_modconv,
            )
            lower_mask = bias_act.bias_act(
                lower_mask,
                self.m_bias2.to(x.dtype),
                clamp=self.conv_clamp,
                act="sigmoid",
            )
...
@persistence.persistent_class
class ToRGBLayerV18_512(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        if self.is_last:
            ...
            self.m_weight1_1 = torch.nn.Parameter(
                torch.randn([1, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
            ...
            self.m_weight2_1 = torch.nn.Parameter(
                torch.randn([1, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
    ...
    def forward(self, x, w, fused_modconv=True):
        ...
        if self.is_last:
            ...
            upper_mask = bias_act.bias_act(
                upper_mask, self.m_bias1.to(x.dtype), clamp=self.conv_clamp
            )
            upper_mask = modulated_conv2d(
                x=upper_mask,
                weight=self.m_weight1_1,
                styles=styles,
                demodulate=False,
                fused_modconv=fused_modconv,
            )
            ...
            lower_mask = bias_act.bias_act(
                lower_mask, self.m_bias2.to(x.dtype), clamp=self.conv_clamp
            )
            lower_mask = modulated_conv2d(
                x=lower_mask,
                weight=self.m_weight2_1,
                styles=styles,
                demodulate=False,
                fused_modconv=fused_modconv,
            )
...
@persistence.persistent_class
class Spade_ResBlockV4_512(nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels.
        out_channels,  # Number of output channels.
...
@persistence.persistent_class
class ToRGBLayerFull_v1_v4(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        if self.is_last and self.is_style:
            self.m_weight1 = torch.nn.Parameter(
                torch.randn([6, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
    ...
    def forward(self, x, w, fused_modconv=True):
        ...
        if self.is_last and self.is_style:
            ...
            pred_parsing = bias_act.bias_act(
                pred_parsing, self.m_bias1.to(x.dtype), clamp=self.conv_clamp
            )
...
@persistence.persistent_class
class ToRGBLayerFull_v1_v5(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        if self.is_last and self.is_style:
            self.m_weight1 = torch.nn.Parameter(
                torch.randn([7, in_channels, kernel_size, kernel_size]).to(
                    memory_format=memory_format
                )
            )
...
@persistence.persistent_class
class SynthesisBlockFull_v1_v4(nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels, 0 = first block.
        out_channels,  # Number of output channels.
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == "skip":
            self.torgb = ToRGBLayerFull_v1_v4(
                out_channels,
                img_channels,
                w_dim=w_dim,
                conv_clamp=conv_clamp,
        ...
        self.spade_b512 = Spade_ResBlockV4_512(
            out_channels, out_channels, spade_channels=1
        )
    ...
    def forward(
        ...
        **layer_kwargs,
        ...
        if self.is_last or self.architecture == "skip":
            y, pred_parsing = self.torgb(
                x, next(w_iter), fused_modconv=fused_modconv
            )
            y = y.to(
...
@persistence.persistent_class
class SynthesisBlockFull_v1_v6(nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels, 0 = first block.
        out_channels,  # Number of output channels.
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == "skip":
            self.torgb = ToRGBLayerFull_v1_v5(
                out_channels,
                img_channels,
                w_dim=w_dim,
                conv_clamp=conv_clamp,
...
@persistence.persistent_class
class SynthesisNetworkFull_v18(nn.Module):
    def __init__(
        self,
        w_dim,  # Intermediate latent (W) dimensionality.
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        assert (
            img_resolution >= 8 and img_resolution & (img_resolution - 1) == 0
        )
        super().__init__()
        self.w_dim = w_dim
        self.img_resolution = img_resolution
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlockFull_v1_v6(
                in_channels,
                out_channels,
                w_dim=w_dim,
                resolution=res,
        ...
        self.spade_b256_1 = Spade_ResBlockV4_512(
            in_channels, out_channels, spade_channels=128
        )
        self.spade_b256_2 = Spade_ResBlockV4_512(
            in_channels, out_channels, spade_channels=128
        )
        ...
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v1_v4(
            in_channels,
            out_channels,
            w_dim=w_dim,
            resolution=res,
        ...
        ngf = 64
        spade_encoder += [
            Conv2dLayer(3, ngf, kernel_size=7, activation="relu")
        ]
        spade_encoder += [
            ResBlock(ngf, ngf, kernel_size=4, activation="relu")
        ]  # 512
        spade_encoder += [
            ResBlock(ngf, ngf * 2, kernel_size=4, activation="relu", down=2)
        ]  # 256
    ...
    def get_spade_feat(self, mask_512, denorm_mask, denorm_input):
        ...
        denorm_mask_256 = torch.nn.functional.interpolate(
            denorm_mask, scale_factor=0.5
        )
        mask_256 = (mask_256 > 0.9).to(mask_512.device).to(mask_512.dtype)
        denorm_mask_256 = (
            (denorm_mask_256 > 0.9).to(mask_512.device).to(mask_512.dtype)
        )

        valid_mask = (
            ((mask_256 + denorm_mask_256) == 2.0)
            .to(mask_512.device)
            .to(mask_512.dtype)
        )
        res_mask = (
            (mask_256 - valid_mask).to(mask_512.device).to(mask_512.dtype)
        )
        ...
        valid_feat_sum = torch.sum(
            spade_denorm_valid_feat, dim=(2, 3), keepdim=True
        )
        valid_mask_sum = torch.sum(valid_mask, dim=(2, 3), keepdim=True)

        valid_index = (
            (valid_mask_sum > 10).to(mask_512.device).to(mask_512.dtype)
        )
        valid_mask_sum = valid_mask_sum * valid_index + (256 * 256) * (
            1 - valid_index
        )
        spade_average_feat = valid_feat_sum / valid_mask_sum

        spade_feat = (
            spade_denorm_feat * (1 - res_mask) + spade_average_feat * res_mask
        )
    ...
    def forward(
        ...
        **block_kwargs,
        ...
        if gt_parsing is not None:
            ...
            parsing_index = torch.argmax(
                softmax(pred_parsing.detach()), dim=1
            )[:, None, ...].float()
        ...
        spade_upper_feat = self.get_spade_feat(
            upper_mask.detach(), denorm_upper_mask, denorm_upper_input
        )
        spade_lower_feat = self.get_spade_feat(
            lower_mask.detach(), denorm_lower_mask, denorm_lower_input
        )

        upper_mask_256 = torch.nn.functional.interpolate(
            upper_mask, scale_factor=0.5
        )
        lower_mask_256 = torch.nn.functional.interpolate(
            lower_mask, scale_factor=0.5
        )
        upper_mask_256 = (
            (upper_mask_256 > 0.9).to(upper_mask.dtype).to(upper_mask.device)
        )
        lower_mask_256 = (
            (lower_mask_256 > 0.9).to(upper_mask.dtype).to(upper_mask.device)
        )

        spade_feat = (
            spade_upper_feat * upper_mask_256
            + spade_lower_feat * lower_mask_256
        )
        ...
        cur_ws = block_ws[-1]
        _, finetune_img, _ = self.texture_b512(
            x_spade_256,
            img_256,
            cur_ws,
            pose_feat,
...
@persistence.persistent_class
class GeneratorFull_v20(nn.Module):
    def __init__(
        self,
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v18(
            w_dim=w_dim,
        ...
        self.const_encoding = ConstEncoderNetwork(
            input_nc=3 + 2, output_nc=512, ngf=64, n_downsampling=6
        )
        self.style_encoding = StyleEncoderNetworkV18(
            input_nc=(10 * 3 + 5 * 3), output_nc=512, ngf=64, n_downsampling=6
        )
    ...
    def forward(
        ...
        **synthesis_kwargs,
        ...
        for i, feat in enumerate(feats):
            h, _ = feat.shape[2], feat.shape[3]
            cat_feats[str(h)] = feat

        img, finetune_img, pred_parsing = self.synthesis(
            ws,
            pose_feat,
            cat_feats,
            denorm_upper_input,
