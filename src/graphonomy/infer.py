...
from src.graphonomy.dataloaders import custom_transforms as tr
from src.graphonomy.networks import deeplab_xception_transfer, graph
...
def decode_labels(mask, num_images=1, num_classes=20):
    ...
    n, h, w = mask.shape
    assert n >= num_images, (
        "Batch size %d should be greater or equal than number of images to \
        save %d."
        % (n, num_images)
    )
...
def inference(
    net, img_path="", output_path="./", output_name="f", use_gpu=True
):
    """
    ...
    for iii, sample_batched in enumerate(
        ...
        inputs, _ = sample_batched[0]["image"], sample_batched[0]["label"]
        ...
        with torch.no_grad():
            ...
            if iii > 0:
                ...
                outputs_final = outputs_final + outputs  # noqa
            else:
