...
from src.graphonomy.networks import graph
...
class Featuremaps_to_Graph(nn.Module):
    def __init__(self, input_channels, hidden_layers, nodes=7):
        ...
        self.weight = Parameter(
            torch.FloatTensor(input_channels, hidden_layers)
        )
        self.reset_parameters()
    ...
    def forward(self, input):
        ...
        weight_node = torch.matmul(
            input1, self.weight
        )  # n x hw x hidden_layer
        # softmax fea_node
        fea_node = F.softmax(fea_node, dim=-1)
        # print(fea_node.size(),weight_node.size())
        graph_node = F.relu(
            torch.matmul(fea_node.transpose(1, 2), weight_node)
        )
...
class Featuremaps_to_Graph_transfer(nn.Module):
    def __init__(
        self, input_channels, hidden_layers, nodes=7, source_nodes=20
    ):
...
class Graph_to_Featuremaps(nn.Module):
    ...
    def __init__(
        self, input_channels, output_channels, hidden_layers, nodes=7
    ):
        super(Graph_to_Featuremaps, self).__init__()
        self.node_fea = Parameter(
            torch.FloatTensor(input_channels + hidden_layers, 1)
        )
        self.weight = Parameter(
            torch.FloatTensor(hidden_layers, output_channels)
        )
        self.reset_parameters()
    ...
    def forward(self, input, res_feature):
        ...
        res_feature_after_view = res_feature.view(
            batch, channeli, hi * wi
        ).transpose(1, 2)
        ...
        new_node = torch.matmul(
            new_fea, self.node_fea
        )  # batch x hw x nodes x 1
...
class Graph_to_Featuremaps_savemem(nn.Module):
    ...
    def __init__(
        ...
        self.node_fea_for_hidden = Parameter(
            torch.FloatTensor(hidden_layers, 1)
        )
        self.weight = Parameter(
            torch.FloatTensor(hidden_layers, output_channels)
        )
    ...
    def forward(self, input, res_feature):
        ...
        # new_fea = torch.cat((res_feature_after_view1,input1),dim=3)
        # sim
        new_node1 = torch.matmul(
            res_feature_after_view1, self.node_fea_for_res
        )
