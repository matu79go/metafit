...
class CoordConv(nn.Module):
    ...
    def __init__(
        self, input_nc, output_nc, with_r=False, use_spect=False, **kwargs
    ):
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        if downsample:
            conv1 = coord_conv(
                input_nc, output_nc, use_spect, use_coord, **kwargs_down
            )
        else:
            conv1 = coord_conv(
                input_nc, output_nc, use_spect, use_coord, **kwargs_fine
            )
        conv2 = coord_conv(
            output_nc, output_nc, use_spect, use_coord, **kwargs_fine
        )
        if norm_layer is None:
            # if isinstance(norm_layer, None):
...
class ResBlockDecoder(nn.Module):
    ...
    def __init__(
        ...
        if upsample:
            ...
            conv2 = spectral_norm(
                nn.Conv2d(
                    hidden_nc, output_nc, kernel_size=3, stride=1, padding=1
                ),
                use_spect,
            )
        if norm_layer is None:
...
class Jump(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = coord_conv(
            input_nc, output_nc, use_spect, use_coord, **kwargs
        )
        if norm_layer is None:
            # if isinstance(norm_layer, None):
            # if type(norm_layer) == type(None):
