...
def parsing2im(self, parsing):
    ...
    for ii in range(size[0]):
        ...
        parsing_numpy = np.zeros(
            (image_index.shape[0], image_index.shape[1], 3)
        )
        ...
        if ii == 0:
            ...
            parsing_tensor = np.concatenate(
                (parsing_tensor, parsing_numpy[np.newaxis, ...]), axis=0
            )
    parsing_tensor = torch.tensor(parsing_tensor)
    parsing_tensor = parsing_tensor / 255.0 * 2.0 - 1.0
...
def label2onehot(parsing_tensor):
    ...
    parsing_label = parsing_label.scatter_(
        1, parsing_tensor.data.long().cuda(), 1.0
    )
...
def random_affine_matrix(output_shape):
    # angle, shift, scale, shear =
    # getRandomAffineParam(output_size=output_shape[2:])
    # center = (output_shape[3] * 0.5 + 0.5, output_shape[2] * 0.5 + 0.5)
    # affm =
    # get_affine_matrix(center, angle, shift, scale, shear).astype(np.float32)
    affm = []
    for i in range(output_shape[0]):
        angle = (random.randint(0, 20) - 10) * math.pi / 180
        transx = np.random.rand() * 0.4 - 0.2
        transy = np.random.rand() * 0.4 - 0.2
        scalex = (np.random.rand() * 0.2 - 0.1) + 1.0
        scaley = (np.random.rand() * 0.2 - 0.1) + 1.0
        affm.append(
            np.array(
                [
                    [scalex * math.cos(angle), math.sin(-angle), transx],
                    [math.sin(angle), scaley * math.cos(angle), transy],
                ],
                dtype=np.float32,
            )
        )
...
def getRandomAffineParam(output_size):
    ...
    if isinstance(shears, (tuple, list)):
        if len(shears) == 2:
            shears = [random.uniform(shears[0], shears[1]), 0.0]
        elif len(shears) == 4:
            shears = [
...
def get_affine_matrix(center, angle, translate, scale, shear):
    matrix_inv = get_inverse_affine_matrix(
...
def apply_random_crop(
    x, valid_coordinates, target_size, scale_range, num_crops=1
):
    # build grid
    bz = x.size(0)
    crop_list = []
    for ii in range(bz):
        ...
        x_ii = x[
            ii : ii + 1,
            :,
            coordinate_ii[2] : coordinate_ii[3] + 1,
            coordinate_ii[0] : coordinate_ii[1] + 1,
        ]
        ...
        flip = (
            torch.round(torch.rand(num_crops, 1, 1, 1, device=x.device)) * 2
            - 1.0
        )
        unit_grid_x = torch.linspace(-1.0, 1.0, target_size, device=x.device)[
            np.newaxis, np.newaxis, :, np.newaxis
        ].repeat(num_crops, target_size, 1, 1)
        ...
        scale = (
            torch.rand(num_crops, 1, 1, 2, device=x.device)
            * (scale_range[1] - scale_range[0])
            + scale_range[0]
        )
        offset = (torch.rand(num_crops, 1, 1, 2, device=x.device) * 2 - 1) * (
            1 - scale
        )
